...
class MonitorManager:
    ...
    def get_history(self, monitor_id, limit=20):
        ...
        try:
            with self.db.postgres_conn.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute(
                    "SELECT status, latency_ms, timestamp FROM monitor_history WHERE monitor_id = %s ORDER BY timestamp DESC LIMIT %s",
                    (monitor_id, limit)
                )
...
def init_databases():
    ...
    print("🚀 Initializing Morphic Backend Database Connections...")
    ...
    # Connect to Redis
    if not db_manager.connect_redis():
        success = False
